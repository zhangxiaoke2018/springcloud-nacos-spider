...
@ActiveProfiles("test")
@Ignore("Handwork")
public class NetworkingIpAddressHelper {
    
    private final static String ipcoolFilePathname = "/Users/xp/temp/ipcool/ipcool";

    @Test
    public void testCIDR() {
        System.out.println(range2cidrlist("5.104.109.160", "5.104.109.191"));
        System.out.println(range2cidrlist("1.0.1.0", "1.0.3.255"));
    }
    
    @Test
    public void testIpCool() throws IOException {
        try (Stream<String> stream = Files.lines(Paths.get(ipcoolFilePathname))) {
            Stream<String> cidr = stream.map(l -> l.split("\\t"))
                .map(a -> range2cidrlist(a[0], a[1]))
                .flatMap(e -> e.stream());
            
            Files.write(Paths.get(ipcoolFilePathname + ".cidr"),  (Iterable<String>)cidr::iterator);
        }
        
    }
    
    public static List<String> range2cidr2(String startIp, String endIp) {         
